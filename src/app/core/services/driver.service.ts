...
export class DriverService {
    ...
    try {
      ...
      if (createAccount && driver.email) {
        ...
        if (userCredential.user) {
          ...
          driver.temporaryPassword = password; // Keep for admin verification
          driver.passwordSetByAdmin = !!adminPassword;
          ...
          // Show success message with password (admin can share directly with driver)
          if (adminPassword) {
            this.toastr.success(
              `Livreur créé avec le mot de passe défini par l'admin. Communiquez le mot de passe: ${password}`,
              "Compte créé",
              { timeOut: 15000 }
            );
          } else {
            ...
            try {
              ...
              this.toastr.success(
                `Livreur créé. Email de réinitialisation envoyé à ${driver.email}`
              );
            } catch (emailError) {
              console.error("Erreur envoi email:", emailError);
              this.toastr.success(
                `Livreur créé. Mot de passe temporaire: ${password}`,
                "Compte créé",
                { timeOut: 15000 }
              );
              this.toastr.warning(
          ...
          return { driverRef, userCredential, temporaryPassword: password };
        }
      } else {
  ...
  async deleteDriverWithAccount(driverId: string): Promise<void> {
    try {
      const driverDocSnapshot = await firstValueFrom(
        this.col.doc(driverId).get()
      );
    ...
    try {
      ...
      if (userCredential.user) {
        ...
        try {
          await this.afAuth.sendPasswordResetEmail(driver.email);
          this.toastr.success(
            `Compte créé pour ${driver.name}. Email de réinitialisation envoyé à ${driver.email}`
          );
          console.log("🔑 Compte ajouté et email envoyé pour:", driver.email);
        } catch (emailError) {
          console.error("Erreur envoi email:", emailError);
          this.toastr.success(
            `Compte créé pour ${driver.name}. Mot de passe temporaire: ${temporaryPassword}`
          );
          this.toastr.warning(
            "Impossible d'envoyer l'email de réinitialisation automatiquement"
          );
        }
  ...
  getDriverByUserId(userId: string): Observable<Driver | undefined> {
    return this.afs
      ...
      .pipe(map((drivers) => (drivers.length > 0 ? drivers[0] : undefined)));
  }
