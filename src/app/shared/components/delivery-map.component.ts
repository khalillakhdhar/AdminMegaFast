...
import { GoogleMapsService, MapOptions } from '../../core/services/google-maps.service';
...
export class DeliveryMapComponent implements OnInit, OnDestroy {
  ...
  @Output() deliveryStatusChange = new EventEmitter<{deliveryId: string, status: string}>();
  ...
  private async loadDeliveries(): Promise<void> {
    // TODO: Remplacer par un vrai service de livraisons
    // Pour l'instant, données de test
    const mockDeliveries: DeliveryMapMarker[] = [
      {
        id: 'delivery-1',
        position: { lat: 36.806, lng: 10.181 },
        type: 'delivery',
        title: 'Livraison #001',
        ...
        deliveryInstructions: 'Sonner à l\'interphone, 2ème étage',
        estimatedArrival: new Date(Date.now() + 15 * 60 * 1000) // +15 min
      },
      {
        id: 'delivery-2',
        position: { lat: 36.810, lng: 10.175 },
        type: 'delivery',
        title: 'Livraison #002',
        deliveryStatus: 'pending',
        priority: 'normal',
    ...
    this.mapState.deliveryMarkers = mockDeliveries;
    this.addDeliveryMarkersToMap();

    // Définir la prochaine livraison
    this.mapState.nextDelivery = mockDeliveries.find(d => d.deliveryStatus === 'pending');
  }
  ...
  private updateUserLocation(position: GeolocationPosition): void {
    ...
    if (this.watchPositionId) {
      // TODO: Intégrer avec LocationTrackingService
    }
  }
  ...
  async startNavigation(delivery: DeliveryMapMarker): Promise<void> {
    ...
    try {
      ...
      // Calculer la route
      const routeInfo = await this.routeCalculationService.calculateRouteWithDirections({
        origin: { lat: this.mapState.currentPosition.lat, lng: this.mapState.currentPosition.lng },
      ...
      if (this.voiceGuidance) {
        this.speakDirection(`Navigation démarrée vers ${delivery.customerInfo?.name}. Distance: ${routeInfo.distance}, durée estimée: ${routeInfo.duration}`);
      }
  ...
  private updateNavigationProgress(position: GeolocationPosition): void {
    ...
    const distance = this.calculateDistance(
      ...
      this.mapState.destination.lat, this.mapState.destination.lng
    );
    ...
    // TODO: Améliorer avec la vraie progression de route
    const totalDistance = parseFloat(this.mapState.routeProgress.distanceRemaining.replace(/[^\d.]/g, ''));
    if (totalDistance > 0) {
      this.mapState.routeProgress.progress = Math.max(0, Math.min(100,
   ...
   */
  updateDeliveryStatus(deliveryId: string, status: 'pending' | 'in_progress' | 'delivered' | 'failed'): void {
    const delivery = this.mapState.deliveryMarkers.find(d => d.id === deliveryId);
    if (delivery) {
  ...
  private moveToNextDelivery(): void {
    const nextDelivery = this.mapState.deliveryMarkers.find(d => d.deliveryStatus === 'pending');
    if (nextDelivery) {
  ...
  private getDeliveryIcon(priority: string, status: string): google.maps.Icon {
    ...
    const color = colors[status as keyof typeof colors][priority as keyof typeof colors.pending] || '#95a5a6';
    ...
    return {
      url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
        <svg width="32" height="40" viewBox="0 0 32 40" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="16" y="20" text-anchor="middle" fill="${color}" font-size="8" font-weight="bold">${priority.charAt(0).toUpperCase()}</text>
        </svg>
      `)}`,
    ...
    return `
      <div class="delivery-info">
        ...
        <div class="customer-info">
          ...
          <i class="fas fa-map-marker-alt"></i> ${delivery.customerInfo?.address}
        </div>
        ...
        <div class="status-priority">
          <span class="badge ${statusClass}">${this.getStatusLabel(delivery.deliveryStatus)}</span>
          <span class="badge ${priorityClass}">${this.getPriorityLabel(delivery.priority)}</span>
        </div>
        <div class="actions">
          <button onclick="this.startNavigation('${delivery.id}')" class="btn btn-primary btn-sm">
            <i class="fas fa-route"></i> Navigation
          </button>
          <button onclick="this.updateStatus('${delivery.id}', 'in_progress')" class="btn btn-warning btn-sm">
  ...
  private centerOnRoute(): void {
    if (!this.map || !this.mapState.currentPosition || !this.mapState.destination) return;

    const bounds = new google.maps.LatLngBounds();
    bounds.extend({ lat: this.mapState.currentPosition.lat, lng: this.mapState.currentPosition.lng });
  ...
  toggleTraffic(): void {
    ...
    // TODO: Implémenter l'affichage/masquage du layer de trafic
  }
  ...
  toggleFollowLocation(): void {
    ...
    if (this.followUserLocation && this.mapState.currentPosition && this.map) {
      this.map.setCenter({ lat: this.mapState.currentPosition.lat, lng: this.mapState.currentPosition.lng });
    }
  }
